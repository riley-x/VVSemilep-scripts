...
def get_pdf(ws, pdf, operator):
    ...
    replace_old = [f"mu-{operator}", f"mu-{operator}-quad"]
...
def joinPOI(filename, outname, operator):
    ...
    new_ws.factory("mu_EFT[0,-100,100]")
    new_ws.factory("expr::muSq('@0*@0',{mu_EFT})")
    new_ws.factory("expr::muLn('@0',{mu_EFT})")

    # Make the new pdf, this is where the real work is
    new_pdf = get_pdf(new_ws, mc.GetPdf(), operator)
    new_mc.SetPdf(new_pdf)
    new_mc.SetParametersOfInterest(new_ws.var("mu_EFT"))
